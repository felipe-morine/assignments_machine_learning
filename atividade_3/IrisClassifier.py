...
def classifier_script():
    ...
    setosa_weight_vector = LogisticClassifier.calculate_weight_vector(iris_training_data, iris_training_setosa_vector)
    versicolor_weight_vector = LogisticClassifier.calculate_weight_vector(iris_training_data, iris_training_versicolor_vector)
    virginica_weight_vector = LogisticClassifier.calculate_weight_vector(iris_training_data, iris_training_virginica_vector)

    # calcula os resultados do  one vs all
    setosa_predicted_results = LogisticClassifier.calculate_results(setosa_weight_vector, iris_test_data)
    versicolor_predicted_results = LogisticClassifier.calculate_results(versicolor_weight_vector, iris_test_data)
    virginica_predicted_results = LogisticClassifier.calculate_results(virginica_weight_vector, iris_test_data)
    ...
    error_rate = LogisticClassifier.error_rate(real_predicted_results, real_test_classes)
    confusion_matrix = LogisticClassifier.conf_matrix(real_predicted_results, real_test_classes)
